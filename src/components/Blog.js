    ...
    useEffect(() => {
        const fetchData = async () =>{
            try{
                ...
                console.log("error from blog featured")
            }
        }
        fetchData();
    ...
    const getBlogs = () =>{
        ...
        <div className="row no-gutters border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative">
            ...
            <p className="card-text mb-auto">{blogpost.excerpt}</p>
                <Link to={`blog/${blogpost.id}`} className="stretched-link">
                    Continue reading
                </Link>
            </div>

            <div className="col-auto d-none d-lg-block">
               <img width= '200' height='250' src = {blogpost.thumbnail} alt="http://www.w3.org/2000/svg"/>
            </div>
        </div>
            )
        })
